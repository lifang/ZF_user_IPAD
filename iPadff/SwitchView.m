...
-(void)initUIWithTag:(int)tag
{
    if (tag == 0) {
        ...
        NSLog(@"第一次创建~~~~~%d",self.subviews.count);
    }else{
    switch (tag) {
        case 445:
            self.backgroundColor = kColor(228, 226, 226, 1.0);
            ...
            for (int i = 0; i < _btnArray.count; i++) {
                ...
                _isChecked = NO;
                [self.btnsArray addObject:btn];
                NSLog(@"点击时候~~~~~%d",self.subviews.count);
            }
            break;
        case 446:
            self.backgroundColor = kColor(228, 226, 226, 1.0);
            ...
            for (int i = 0; i < _btnArray.count; i++) {
                ...
                if (btn.tag == 446) {
                    btn.frame = CGRectMake(20 + i * btnWidth, mainBtnY, btnWidth, btnHeight);
                    [btn setBackgroundImage:[UIImage imageNamed:@"chose_Btn"] forState:UIControlStateNormal];
                    btn.titleLabel.font = [UIFont systemFontOfSize:20];
                }
                [self addSubview:btn];
